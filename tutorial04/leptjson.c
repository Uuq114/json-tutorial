    ...
    *u = 0;
    for (int i = 0; i < 4; ++i) {
        ...
        *u <<= 4;
        ...
        else if (ch >= 'A' && ch <= 'F') *u += ch - 'A' + 10; /*I forgot add "+ 10" here before*/
        else return NULL;
    }
    return p;
}
                ...
                switch (*p++) {
                    ...
                    case 'u':
                        ...
                        if (u >= 0xD800 && u <= 0xDBFF) {
                            ...
                            unsigned low;
                            if (!(p = lept_parse_hex4(p, &low))) STRING_ERROR(LEPT_PARSE_INVALID_UNICODE_HEX);
                            if (low < 0xDC00 || low > 0xDFFF) STRING_ERROR(LEPT_PARSE_INVALID_UNICODE_SURROGATE);
